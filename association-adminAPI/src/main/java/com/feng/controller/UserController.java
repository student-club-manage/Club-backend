...
public class UserController {
    ...
    @ApiOperation("通过name,role获取user列表")
    @GetMapping("/getByName_Role/{name}/{role}")
    public ResponseResult getByName_Role(@PathVariable("name") String name, @PathVariable("role") Integer role) {
//        User user = userService.getById(id);
        List<User> users = userService.getByName_Role(name, role);
        return ResponseResultUtil.renderSuccess(users);
    }
    ...
    @ApiOperation("用户登录接口1，返回用户")
    @PostMapping("/login1")
    public ResponseResult login1(@Valid @RequestBody LoginUserVo userVo, BindingResult bindingResult, HttpServletRequest request, HttpServletResponse response) {
        if (bindingResult.hasErrors()) {
            String msg = bindingResult.getFieldError().getDefaultMessage();
            log.error("{}", msg);
            throw new ParamInvalidException(ErrorEnum.INVALIDATE_PARAM_EXCEPTION.setMsg(msg));
        ...
        return ResponseResultUtil.renderSuccess(loginUser);
    }
